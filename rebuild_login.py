...
replacement = '''    // Auth gate — must sign in
    if (!user) {
      return (
        <div style={{
          display: 'flex',
...
# Find the old auth gate (which uses className="authGate") and replace it until the Toaster is done
# We'll use a regex that captures everything from if (!user) { until the end of the if block.

pattern = re.compile(r'\s*//\s*Auth\s*gate\s*—\s*must\s*sign\s*in\s*if\s*\(!user\)\s*\{.*?return\s*\(\s*<div\s+className="claudeShell', re.DOTALL)

# Let's verify we can find it
match = pattern.search(text)
if match:
    # Instead of replacing until claudeShell, we just replace the if block.
    # Actually wait! The regex captures UP TO return ( <div className="claudeShell
    # So we should append \n  return (\n    <div className="claudeShell to our replacement.
    
    final_replacement = replacement + '\n\n  return (\n    <div className={claudeShell'
    
    new_text = text[:match.start()] + final_replacement + text[match.end() - 12:]
    
    # Actually let's just make it simple using text.find
    idx1 = text.find('// Auth gate')
    if idx1 == -1: idx1 = text.find('if (!user) {')
    ...
    if idx1 != -1 and idx2 != -1:
        ...
        print("Couldn't find exact bounds")
else:
    # Manual fallback
    idx1 = text.find('if (!user) {')
    # search for the end of the if block
    # We know the next thing is the main return
    idx2 = text.find('  return (\n    <div className={claudeShell', idx1)
    if idx2 == -1: idx2 = text.find('  return (\n    <div className="claudeShell', idx1)
    
    if idx1 != -1 and idx2 != -1:
        # Move idx1 to grab the comment too
        comment_idx = text.rfind('// Auth gate', 0, idx1)
        if comment_idx != -1 and comment_idx > idx1 - 50:
            idx1 = comment_idx
            
        new_text = text[:idx1] + replacement + '\n\n' + text[idx2:]
        with open('frontend/src/App.jsx', 'w', encoding='utf-8', newline='\n') as f:
            f.write(new_text)
        print("Restored JSON UI formatting WITH logo successfully (fallback)!")
    else:
